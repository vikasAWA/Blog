...
def rows_from_file(
    ...
    .. code-block:: python
        ...
        from fastlite.utils import rows_from_file
        import io
    ...
    to return dictionaries, while ``format_used`` is a value from the ``fastlite.utils.Format`` enum:
    ...
    ``fastlite.utils.RowError`` exception will be raised when you loop over the generator.
...
class TypeTracker:
    ...
    .. code-block:: python
        ...
        from fastlite.utils import TypeTracker
        import fastlite

        db = fastlite.Database(memory=True)
        tracker = TypeTracker()
...
def hash_record(record: Dict, keys: Optional[Iterable[str]] = None):
    ...
    Example usage::
        ...
        from fastlite.utils import hash_record
